var alm_woo={};(function(){alm_woo.init=true;alm_woo.paging=false;alm_woo.previousUrl=window.location.href;alm_woo.isAnimating=false;alm_woo.defaultPage=1;alm_woo.fromPopstate=false;alm_woo.HTMLHead=document.getElementsByTagName("head")[0].innerHTML;alm_woo.timer=null;alm_woo.isIE=navigator.appVersion.indexOf("MSIE 10")!==-1?true:false;window.almWooCommerce=function(alm){if(!alm.addons.woocommerce){return false;}
if(alm_woo.init){alm_woo.scrolltop=alm.addons.woocommerce_settings.settings.scrolltop?parseInt(alm.addons.woocommerce_settings.settings.scrolltop):50;alm_woo.controls=alm.addons.woocommerce_settings.settings.controls==="true"?true:false;alm_woo.container=document.querySelector(alm.addons.woocommerce_settings.container);alm_woo.first=alm_woo.container?alm_woo.container.querySelector(".alm-woocommerce:first-child"):"";alm_woo.paged_urls=alm.addons.woocommerce_settings.paged_urls;alm_woo.paging=alm.addons.paging;}
...
if(alm_woo.container){alm_woo.scrollToPage(page);}};alm_woo.scrollToPage=function(page){page=page===undefined||page===""||page==="-1"||page===-1?alm_woo.first.dataset.page:page;if(alm_woo.isAnimating){return false;}
...
setTimeout(function(){alm_woo.isAnimating=false;},250);};alm_woo.getRelLinks=function(page){page=parseInt(page);alm_woo.setRelLink(page-1,"prev");alm_woo.setRelLink(page+1,"next");};alm_woo.setRelLink=function(page,type){var rel=document.querySelector('link[rel="'+type+'"]');if(type==="next"&&page>alm_woo.paged_urls.length){alm_woo.removeRelLink(rel);return false;}
if(type==="prev"&&page==0){alm_woo.removeRelLink(rel);return false;}
var link=alm_woo.paged_urls[page-1];if(rel){rel.href=link;}else{rel=document.createElement("link");rel.href=link;rel.rel=type;document.getElementsByTagName("head")[0].appendChild(rel);}};alm_woo.removeRelLink=function(rel){if(rel){rel.parentNode.removeChild(rel);}};})();
